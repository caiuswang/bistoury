...
LOCAL_IP=`/sbin/ifconfig -a eth0|grep inet|grep -v 127.0.0.1|grep -v inet6|awk '{print $2}'|tr -d "addr:"|tail -1`

start(){

    cd $H2_DATABASE_DIR
    ./h2.sh -j /usr/local/java/default/ -i $LOCAL_IP -l $APP_LOG_DIR start
    sleep 5
...
while getopts i:j:l:c:h opt;do
    case $opt in
#        p) APP_PID=$OPTARG;;
        i) LOCAL_IP=$OPTARG;;
        j) JAVA_HOME=$OPTARG;;
...
if [[ "start" == $CMD ]] && [[  -n "$JAVA_HOME" ]]; then
    ...
    start  "pid" $JAVA_HOME $BISTOURY_AGENT_APP_LIB_CLASS $LOCAL_IP
elif [[ "stop" == $CMD ]]; then
    stop
