...
# Optimized for use with version 3.5.44 of the Wrapper.
#
...
# local java additionl
LOCAL_JAVA_ADDITIONAL_FILE="/data/wrapper/%app.name%/java.additional.conf"
LOCAL_CONFIG=""
if [ -f $LOCAL_JAVA_ADDITIONAL_FILE ]; then
  LOCAL_CONFIG="wrapper.java.additional_file=\"$LOCAL_JAVA_ADDITIONAL_FILE\""
  echo -e "\033[33m enable local java additional $LOCAL_JAVA_ADDITIONAL_FILE\033[0m"
fi
...
start() {
    ...
    # The string passed to eval must handles spaces in paths correctly.
    COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" $LOCAL_CONFIG wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.daemonize=TRUE $APPNAMEPROP $ANCHORPROP $IGNOREPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.44 $ADDITIONAL_PARA"
    echo $COMMAND_LINE
    eval $COMMAND_LINE
